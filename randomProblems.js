...
async function fetchProblems(tag) {
  const url = `https://solved.ac/api/v3/search/problem?query=tag:${tag}&sort=random&direction=asc`;
  try {
...
async function getRandomProblems() {
  ...
  if (uniqueProblems.length >= 3) {
    uniqueProblems.sort(() => Math.random() - 0.5); // 랜덤 섞기
    uniqueProblems = uniqueProblems.slice(0, 3); // 3문제 선택
  }

  console.log("🎯 랜덤으로 선택된 알고리즘 문제:");
